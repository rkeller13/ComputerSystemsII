...
//  PURPOSE:  To tell how many Dish instances that each Chef instance prepares,
...
class		Table
{
  ...
  pthread_cond_t toClear_;
  pthread_cond_t toServe_;
...
public :
  ...
  Table		() :
  		dishPtr_(NULL)
		{
		  //  PERHAPS INITIALIZE SOME VARS 
      pthread_mutex_init(&lock_, NULL);
      pthread_cond_init(&toClear_, NULL);
      pthread_cond_init(&toServe_, NULL);
		}
  ...
  ~Table	()
      ...
      pthread_cond_destroy(&toClear_);
      pthread_cond_destroy(&toServe_);
		}
  ...
  void		serve		(Chef&		chef,
      ...
      while (dishPtr_ != NULL) {
        std::cout 
        << chef
        << ": Get this dish off the freakin table!\n";
        pthread_cond_wait(&toClear_, &lock_);
      }
  ...
  Dish*		eatFrom		(Gourmand&		gourmand
      ...
      while (dishPtr_ == NULL) {
        ...
        pthread_cond_wait(&toServe_, &lock_);
      }
			...
			)
{
  ...
  for (int i = 0; i < NUM_DISHES_TO_DO; i++) {
    Dish* dishPtr  = chefPtr->prepare();
    table.serve(*chefPtr,dishPtr);
  }


  return(chefPtr /* CHANGE IF YOU WANT */);
}
			...
			)
{
  ...
  //  PERHAPS A LOOP HERE
  for (int i = 0; i < NUM_DISHES_TO_DO; i++) {
    Dish* dishPtr = table.eatFrom(*gourmandPtr);
    gourmandPtr->consume(dishPtr);
  }

  return(gourmandPtr /* CHANGE IF YOU WANT */);
}
			...
			)
{
  ...
  for (int i = 0; i < NUM_CHEFS; i++) {
    pthread_create(&chefIds[i], NULL, cook, new Chef(i));
  }

  for (int j = 0; j < NUM_GOURMANDS; j++) {
    pthread_create(&gourmandIds[j], NULL, eat, new Gourmand(j));
  }
  ...
  for (int j = 0; j < NUM_CHEFS; j++) {
    Gourmand* gourmandPtr;
    pthread_join(gourmandIds[j], (void**)&gourmandPtr);
    delete(gourmandPtr);
  }
